...
//    if (ProgressDialog) qDebug()<<"readyread..."<<ProgressDialog->value();
    if (DoingCheck)
    {
        ...
        //qDebug()<<"Version: "<<version<<version.toFloat();
        if (version.toFloat()>=(UPDATEVERSION+.0001))
        {
                //QMessageBox::information(0,"Update","Here3");
                MacBodgeClickedNoForUpdateDownload=true;
                //DownloadNeeded=true;
                DownloadURL = reply->readLine();
                if (QMessageBox::question(0,"SPIERS update","A new version of SPIERS is available.\nDo you want to download it?\n\nNote that after downloading this program will close before running the new installer.",QMessageBox::No,QMessageBox::Yes)==QMessageBox::Yes)
                {
                    #ifndef __APPLE__
                    NetModule n2;
...
//                    qDebug()<<n2.DownloadError;
                    ...
                    {
                        QMessageBox::warning(0,"SPIERS update error","File does not appear to have downloaded properly - this may be a virus-checker/firewall issue. Visit the SPIERS website (www.spiers-software.org) to manually download the latest version");
                        return;
                    }
                    ...
                    #endif
                #ifdef __APPLE__
                    QMessageBox::information(0,"Update","Updater wil be downloaded via your web-browser. Once downloaded run it manually if it does not start automatically. Close all SPIERS applications before installing!");
        ...
        if (readcopy.left(3)=="(c)" || readcopy.left(3)=="(C)")
        {
            ...
            for (int i=0; i<i_comm->length(); i++)
            {
               if (i_comm->at(i).left(3)=="(c)" || i_comm->at(i).left(3)=="(C)")
                { i_comm->replace(i,readcopy); return;}
