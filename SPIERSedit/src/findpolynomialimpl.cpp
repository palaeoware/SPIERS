...
double findpolynomialImpl::PolyTweak(double val)
{
    ...
    rval = static_cast<double>(rand255()) / 32768; //Should be +- 0-1
...
void findpolynomialImpl::Breed()
{
    ...
    do
    {
        ...
        c += 250;
        MyOutString = "";
        if (runsdone == 0)out << "Run " << runsdone + 1 << " Gen " << c << "\n Fitness: " << fit << " (" << static_cast<int>(static_cast<double>(fit * 100) / static_cast<double>
                                                                                                                                 (Sample.count() / seg->PolySparse)) << "%)";
        else out << "Run " << runsdone + 1 << " Gen " << c << "\n Fitness: " << fit << " (" << static_cast<int>(static_cast<double>(fit * 100) / static_cast<double> (Sample.count() / seg->PolySparse))
                     << "%)" << "\n" << "  Best: " << bestfit << " (" << static_cast<int>(static_cast<double>(bestfit * 100) / static_cast<double>(Sample.count() / seg->PolySparse)) << "%)";
        ...
        if (pleasestop == true)
        {
            ...
            if (runsdone == 0)
            {
                bestfit = fit;
                MyOutString = "";
                out << "Polynomial fit: " << "\n" << bestfit << " (" << static_cast<int>(static_cast<double>(bestfit * 100) / static_cast<double>(Sample.count() / seg->PolySparse)) << "%)";
                label->setText(LabelBaseString + MyOutString);
                return;
            }
            else
...
void findpolynomialImpl::find()
{
                    ...
                    if (Locks[invertedpos * 2])
                    {
                        ...
                        if  (MasksSettings[static_cast<quint8>(Masks[invertedpos])]->Show) //don't include hidden masks
                        {
                            ...
                            if (GreyImage) //repeat same value 3 times - not ideal, but hardly going to be used for GS anyway!
                            {
                                ...
                                Sample.append(SampleEntry(static_cast<uchar>(r), static_cast<uchar>(r), static_cast<uchar>(r), SegmentMap[y * fwidth + x]));
                            }
                            else
                            {
                                for (int x2 = x * ColMonoScale; x2 < x * ColMonoScale + ColMonoScale; x2++)
                                ...
                                Sample.append(SampleEntry(static_cast<uchar>(r), static_cast<uchar>(g), static_cast<uchar>(b), SegmentMap[y * fwidth + x]));
                            }
                        }
                    }
                }
    ...
    if (noseg == 0)
    {
        Message("Select some pixels using select/lock as a sample, select appropriate slices, then try again!");
        return;
    }
    for (int i = 0; i < SegmentCount; i++)
        noseg -= segcounts[i];
