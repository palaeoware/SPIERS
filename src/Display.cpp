...
//        qDebug()<<GA[0]->size();
    ...
    if (CurrentMode==3) lockmode = true; else lockmode = false;
    if (CurrentMode==1) maskmode = true; else maskmode = false;
	if (ThreshFlag) greymode=false; else greymode=true; 
	...
	else
	{
        ...
        for (x=0; x<fwidth; x++)
        {
            ...
            if (!(MasksSettings[(quint8)Masks[invertedpos]]->Show))
            {
                ...
                {
				    RED(data,m)=(uchar)0; GREEN(data,m)=(uchar)0; BLUE(data,m)=(uchar)0;
				}
			}
            else
            {
			    ...
			    if (lockmode)
                {
				    if (Locks[invertedpos*2]) //*2 as these are qint16's. Just look at one byte is fine though - bools!
                    {
						...
						{
                            RED(data,m) = (uchar)0; GREEN(data,m)=(uchar)0; BLUE(data,m)=(uchar)100;
                        }
                    }
					else
                    {
...
//Does ALL preprocessing of image, which has already been loaded from source data
	...
	tcwidth=fwidth*ColMonoScale; tcheight=fheight*ColMonoScale;  //now use fwidth, fheight
	...
	{	
		...
		{		
			for (j=0; j<cwidth; j++)
			{
				...
				if (Col>255) Col=255; if (Col<0) Col=0;
				Col*=Transf;
				...
				if (Col2>255) Col2=255; if (Col2<0) Col2=0;
...
void MakeRangeGreyScale(int seg, int fnum, bool flag=false)
{
	...
	double g;
	if (b==t) g=500; else g=(127.0)/(((double)(t-b))/2.0);
...
double CalcPoly(unsigned char r, unsigned char g, unsigned char b, Segment *seg)
{
	...
	redp=red=(double)r; 	greenp=green=(double)g;		bluep=blue=(double)b;
	...
	for (n=0; n<check; n++)
	{
		...
		greenp *= green; redp *= red; bluep *= blue;
	}
...
uchar RangePixel(int w, int h, int bot, int top, double cen, double gra, int seg)
{
    ...
    else
    {
        ...
        if (val < 0) return 0; else return (uchar) (val+.5);
    }
}
...
uchar GenPixel(int x, int y, int s)
{
	...
	{ 
	   ...
	   if (Segments[s]->LinInvert) return 255-t; else return t;
    }
