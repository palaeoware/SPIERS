...
bool MyThread::TryCaching(int fnumber)
{
	if (CacheIndex(fnumber)==-1)
	{
		...
		if (CacheMode==0)
		{
			if (m->size()==0) {Caches[Caches.count()-1]->Masks=0; delete m;} else {Caches[Caches.count()-1]->Masks= mcomp; if (mcomp!=m) delete m;}
			if (m2->size()==0) {Caches[Caches.count()-1]->Locks= 0; delete m2;} else {Caches[Caches.count()-1]->Locks= m2comp; if (mcomp!=m) delete m;}		
		}
		else
		{
			if (m->size()==0) 
...
void MyThread::TimerFired()
{
	...
	QMutexLocker locker(&mutex);	
 	if (BackgroundCacheFilling && Active)
 	{
		...
		if (SizeTotal>=((long long int)CacheMem*(long long int)(1024*1024))) {	return;}
