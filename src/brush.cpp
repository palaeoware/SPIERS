...
// 3 is 3d 3d ellipsoid brush
{
	...
	Radius=size;
	//clear everything
	Xpos.clear(); Ypos.clear(); Soft.clear(); PixelCount=0;
	LeftLinesX.clear(); RightLinesX.clear(); TopLinesX.clear(); BottomLinesX.clear();
	LeftLinesY.clear(); RightLinesY.clear(); TopLinesY.clear(); BottomLinesY.clear();
	LeftLinesCount=0; RightLinesCount=0;TopLinesCount=0;BottomLinesCount=0;
	...
	if (shape==0) //square
	{
		...
		for (y=0-(size-1); y<size; y++)	
		{
			if (x==(0-(size-1))) 
			{
				LeftLinesX << x; LeftLinesY << y; LeftLinesCount++;
			}
			if (x>=(size-1)) 
			{
				RightLinesX << x; RightLinesY << y; RightLinesCount++;
			}				
			Xpos << x; Ypos << y; PixelCount++;
		}
		
		//now put in tops and bottoms
		for (x=0-(size-1); x<size; x++)	
		{
			TopLinesX << x; TopLinesY << (0-(size-1)); TopLinesCount++;
			BottomLinesX << x; BottomLinesY << (size-1); BottomLinesCount++;
		}
	...
	else if (shape==1) //circle
	{
		...
		if (size==2) 
		{	//cross
			LeftLinesX << -1; LeftLinesY << 0; LeftLinesCount++;
			LeftLinesX << 0; LeftLinesY << 1; LeftLinesCount++;
			LeftLinesX << 0; LeftLinesY << -1; LeftLinesCount++;
			RightLinesX << 1; RightLinesY << 0; RightLinesCount++;
			RightLinesX << 0; RightLinesY << 1; RightLinesCount++;
			RightLinesX << 0; RightLinesY << -1; RightLinesCount++;
			TopLinesX << 0; TopLinesY << -1; TopLinesCount++;
			TopLinesX << -1; TopLinesY << 0; TopLinesCount++;
			TopLinesX << 1; TopLinesY << 0; TopLinesCount++;
			BottomLinesX << 0; BottomLinesY << 1; BottomLinesCount++;
			BottomLinesX << -1; BottomLinesY << 0; BottomLinesCount++;
			BottomLinesX << 1; BottomLinesY << 0; BottomLinesCount++;
			Xpos<<0; Ypos<<-1;
			Xpos<<0; Ypos<<0;
			Xpos<<0; Ypos<<1;
		...
		{	//cross
			LeftLinesX << -1; LeftLinesY << -2; LeftLinesCount++;
			LeftLinesX << -2; LeftLinesY << -1; LeftLinesCount++;
			LeftLinesX << -2; LeftLinesY << 0; LeftLinesCount++;
			LeftLinesX << -2; LeftLinesY << 1; LeftLinesCount++;
			LeftLinesX << -1; LeftLinesY << 2; LeftLinesCount++;

			RightLinesX << 1; RightLinesY << -2; RightLinesCount++;
			RightLinesX << 2; RightLinesY << -1; RightLinesCount++;
			RightLinesX << 2; RightLinesY << 0; RightLinesCount++;
			RightLinesX << 2; RightLinesY << 1; RightLinesCount++;
			RightLinesX << 1; RightLinesY << 2; RightLinesCount++;

			TopLinesX << -2; TopLinesY << -1; TopLinesCount++;
			TopLinesX << -1; TopLinesY << -2; TopLinesCount++;
			TopLinesX << 0; TopLinesY << -2; TopLinesCount++;
			TopLinesX << 1; TopLinesY << -2; TopLinesCount++;
			TopLinesX << 2; TopLinesY << -1; TopLinesCount++;

			BottomLinesX << -2; BottomLinesY << 1; BottomLinesCount++;
			BottomLinesX << -1; BottomLinesY << 2; BottomLinesCount++;
			BottomLinesX << 0; BottomLinesY << 2; BottomLinesCount++;
			BottomLinesX << 1; BottomLinesY << 2; BottomLinesCount++;
			BottomLinesX << 2; BottomLinesY << 1; BottomLinesCount++;
			...
			for (y=0-(size-1); y<size; y++)	
			{
				...
				for (x=0-(size-1); x<size; x++)	
				{
					...
					if (d2<d) //inside
					{
						if (!flag) //first one
						{
							LeftLinesX << x; LeftLinesY << y; LeftLinesCount++; flag=true;
						}
						Xpos << x; Ypos << y; PixelCount++;
					}
					...
					{
						flag=false;
						RightLinesX << (x-1); RightLinesY << y; RightLinesCount++;
					}
				}
				//if flag is still true need to set right still
				if (flag) 	{RightLinesX << (x-1); RightLinesY << y; RightLinesCount++; flag=true;}
			...
			for (x=0-(size-1); x<size; x++)	
			{
				...
				for (y=0-(size-1); y<size; y++)	
				{
					...
					if (d2<d) //inside
					{
						if (!flag) //first one
						{
							TopLinesX << x; TopLinesY << y; TopLinesCount++; flag=true;
						}
					}
					else if (flag) //run off end - LAST pixel should be edge
					{
						flag=false;
						BottomLinesX << x; BottomLinesY << (y-1); BottomLinesCount++;
					}
				}
				//if flag is still true need to set right still
				if (flag) 	{BottomLinesX << x; BottomLinesY << (y-1); BottomLinesCount++; flag=true;}
	...
	else if (shape==2 || shape==3)
	{
		...
		for (int x=0-max; x<max; x++)
		{
			...
			if (TY.count())
			{
				...
				bool on=false; 	int last=TY[0]-2;
				foreach (int y, TY)
				{
					if (y!=last+1)
					{
						//top line
						TopLinesX << x; TopLinesY << y; TopLinesCount++;
						if (on) //also need to close last one
							{BottomLinesX << x; BottomLinesY << last; BottomLinesCount++;}
					}
					last=y; on=true;				
				}
				//and closing line
			...
			if (TX.count())
			{
				...
				foreach (int x, TX)
				{
					if (x!=last+1)
					{
						...
						LeftLinesX << x; LeftLinesY << y; LeftLinesCount++;
						if (on) //also need to close last one
							{RightLinesX << last; RightLinesY << y; RightLinesCount++;}
					}
					last=x;
					on=true;				
				}
...
//returns true if anything drawn, otherwise false (allows cursor to change)
{
	...
	if (!NoRecreateFlag)
	{
		...
		if (!RestartFlag) foreach(temp, LineList) {scene->removeItem(temp);}
	}
	RestartFlag=false;
	...
	{
		lx=LeftLinesX[n]+(x/ColMonoScale); ly=LeftLinesY[n] + (y/ColMonoScale);

		if (lx>=0 && lx<(fwidth) && ly>=0 && ly<(fheight)) //pixel is in range
		{
			rx1=(qreal)lx * rcms; 	rx2=rx1; 	
			ry1=(qreal)ly * rcms; 	ry2=((qreal)ly + 1) *rcms;
 			if (NoRecreateFlag) {LineList[counter]->setLine (rx1, ry1, rx2, ry2); LineList[counter++]->setVisible(true);}
			else {temp = scene->addLine( rx1,ry1, rx2, ry2, MyPen); temp->setZValue(1); 	LineList.append(temp);}
		}
	}

	for (n=0; n<RightLinesCount; n++)
	{
		lx=RightLinesX[n]+(x/ColMonoScale); ly=RightLinesY[n] + (y/ColMonoScale);

		if (lx>=0 && lx<(fwidth) && ly>=0 && ly<(fheight)) //pixel is in range
		{
			rx1=((qreal)lx+1) * rcms; 	rx2=rx1; 	
			ry1=(qreal)ly * rcms; 	ry2=((qreal)ly + 1) *rcms;
 			if (NoRecreateFlag) {LineList[counter]->setLine (rx1, ry1, rx2, ry2); LineList[counter++]->setVisible(true);}
			else {temp = scene->addLine( rx1,ry1, rx2, ry2, MyPen); temp->setZValue(1); 	LineList.append(temp);}
		}
	...
	{
		lx=BottomLinesX[n]+(x/ColMonoScale); ly=BottomLinesY[n] + (y/ColMonoScale);

		if (lx>=0 && lx<(fwidth) && ly>=0 && ly<(fheight)) //pixel is in range
		{
			rx1=(qreal)lx * rcms; 	rx2=((qreal)lx + 1) *rcms; 	
			ry1=((qreal)ly+1) * rcms; 	ry2=ry1;
 			if (NoRecreateFlag) {LineList[counter]->setLine (rx1, ry1, rx2, ry2); LineList[counter++]->setVisible(true);}
			else {temp = scene->addLine( rx1,ry1, rx2, ry2, MyPen); temp->setZValue(1); 	LineList.append(temp);}
		}
	}
	...
	{
		lx=TopLinesX[n]+(x/ColMonoScale); ly=TopLinesY[n] + (y/ColMonoScale);

		if (lx>=0 && lx<(fwidth) && ly>=0 && ly<(fheight)) //pixel is in range
		{
			rx1=(qreal)lx * rcms; 	rx2=((qreal)lx + 1) *rcms; 	
			ry1=((qreal)ly) * rcms; 	ry2=ry1;
 			if (NoRecreateFlag) {LineList[counter]->setLine (rx1, ry1, rx2, ry2); LineList[counter++]->setVisible(true);}
			else {temp = scene->addLine( rx1,ry1, rx2, ry2, MyPen); temp->setZValue(1); 	LineList.append(temp);}
		}
	}
					...
					if (flag)
					{
		                ...
		                for (int i=0; i<SegmentCount; i++)
						{
							if (Segments[i]->Activated)
							{
								...
								if (temp >= high)  { high = temp; seg = i; sflag=Segments[seg]->Locked;}
							}
						}
	...
	{	
		...
		if (ay<fheight)
		{
			...
			{	
				...
				if (effect) data[pos]=255; else data[pos]=0;
			}
		}
	}	
	LocksDirty=true;
...
void Brush_class::segment(int x, int y, int effect)
{
	...
	if (effect) m=SelectedMask; else m=SelectedRMask;
	xoff=x/ColMonoScale;
	yoff=y/ColMonoScale;
	
	if (effect) s=CurrentSegment; else s=CurrentRSegment;
	            ...
	            else //not delete - normal segment mode
	            {
	                ...
	                for (n=0; n<SegmentCount; n++)
	                    	if (Segments[n]->Activated) 
	                    		{if ((data[n])[pos]>=max) 
	                    		{smax=n+1; max=(data[n])[pos];flag=Segments[n]->Locked;}
                    			}
	                ...
	                if (!flag)		
					{
						...
						{
							if (smax==0) {(data[s])[pos]=(uchar)255;}
	                    	else
	                    	{
