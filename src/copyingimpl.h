...
public:
	...
	void DoOutputRecursive(QList <int> *translationtable, int *nexttransentry, QList <int> *translationtable2, int *nexttransentry2, int parent, long long int asize, int filesused, int awidth, int aheight, QDataStream *out, QList <double> *stretches);
...
private:
	...
	bool DoIHaveChildren(int parent);
        void MakeMergeObject(int *, QByteArray *, QVector <double> *TrigArray, int *TrigCount, int parent, long long int asize, int filesused, int awidth, int aheight, QDataStream *out, QList <double> *stretches);
