...
void CopyingImpl::MakeMergeObject(int *FullOutArrayCount, QByteArray *FullOutArray, QVector <double> *TrigArray, int *TrigCount, int parent, long long int asize, int filesused, int awidth, int aheight, QDataStream *out, QList <double> *stretches)
{
...
void CopyingImpl::DoOutputRecursive(QList <int> *translationtable, int *nexttransentry, QList <int> *translationtable2, int *nexttransentry2, int parent, long long int asize, int filesused, int awidth, int aheight, QDataStream *out, QList <double> *stretches)
...
//Just don't ask
{
        ...
        //find lowest
                ...
                {	// for each object to be output;
                        ...
                        {
                                if 	(OutputObjects[i]->Merge) //merged group - all children merged into this one object
                                tt->append(i);  else StupidBodgyFunction(tt, i);
                        }
...
//Adapted from ExportSPV - does same work, but doesn't export arrays, just counts contents
{
    ...
    if (ActualOutputObjectCount==0) {Message("Nothing to Count!"); return;}
    ...
    filesused = (filesused - filesused % ZDownsample) / ZDownsample;
        ...
        QList <int> translationtable; for (int i=0; i<OutputObjectsCount; i++) translationtable.append(0);
        int nexttransentry=0, nexttransentry2=0;
        QList <int> translationtable2; for (int i=0; i<OutputObjectsCount; i++) translationtable2.append(0);
...
void CopyingImpl::ExportSPV(int flag)  //0 for export, 1 for export and launch, 2 for export to temp file and launch
{
    ...
    int awidth, aheight;
        ...
        //qDebug()<<"1";
    int ActualOutputObjectCount=translationtable1.count();
    if (ActualOutputObjectCount==0) {Message("Nothing to Export!"); return;}
    ...
    //for (n=0; n<=filesused+1; n++) stretches.append(n);
    stretches.append(0);
    if (FirstOutputFile==0) stretches.append(0); else stretches.append(Stretches[FirstOutputFile-1]);
    ...
    else
    {
        ...
        {	tot=stretches[spos++]; //actually a max
                zpos++; if (zpos==ZDownsample) {stretches2.append(tot); zpos=0;};
        }
   ...
   //sort out downsampled stretches
    ...
    if (!File.open(QIODevice::WriteOnly)) {Message("Sorry, can't open spv file for writing"); return;}
    ...
    //had a reverse here - I think it was superflous?
        ...
        for (int i=0; i<nexttransentry; i++)
                if (OutputObjects[translationtable[i]]->Parent==-1) out<<(int)-1; else out<<translationtable.indexOf(OutputObjects[translationtable[i]]->Parent);
        for (int i=0; i<nexttransentry; i++)
                if (OutputObjects[translationtable[i]]->IsGroup && OutputObjects[translationtable[i]]->Merge==false ) out<<(uchar) 1; else out<<(uchar) 0;

        for (int i=0; i<nexttransentry; i++)
        //these are the 'show groups' output - leave as per isgroup for now
                if (OutputObjects[translationtable[i]]->IsGroup) out<<(uchar) 1; else out<<(uchar) 0;

        //keys
        //nextkey=1;
        for (int i=0; i<nexttransentry; i++)
                if (OutputObjects[translationtable[i]]->Key==0) out<<(uchar)1; else out<<(uchar) OutputObjects[translationtable[i]]->Key;
...
//        qDebug()<<"Start loop";
        ...
        QString message;   QTextStream ms(&message);
        ms<<"File: "<<f;
               ...
               foreach(Object, outlist)
                {
                    ...
                    OutputArray = &(Object->Outputarray); temparray = &(Object->temparray);
...
//                        qDebug()<<"H3c";
                        UseMasks= &(Obj->UseMasks); UseSegs=&(Obj->UseSegs);
...
//                qDebug()<<"H4b";
              ...
              {
                   int ymax=m*GRID_SCALE+GRID_SCALE; if (ymax>aheight) ymax=aheight; //height of cell (might be less than grid size at end)
                   for (n=0; n<gxsize; n++)  //for each column
                   {
                       int xmax=n*GRID_SCALE+GRID_SCALE; if (xmax>awidth) xmax=awidth; //width of cell
