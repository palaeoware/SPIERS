...
SPIERSedit 2: curves.cpp

Functions relating to curve system 
...
void AddNode()
{
    ...
    if (Curves[SelectedCurve]->SplinePoints[CurrentFile* zsparsity]->Count == 0)
    {
        ...
        CurvesDirty=true; CurvesUndoDirty=true;
    ...
    if (Curves[SelectedCurve]->Closed) //closed loop
    {
        for (sn=0; sn< p->Count; sn++)  //all the line segments
        {
        	for (sm=1; sm<10; sm++)
        	{
                ...
                if (d < mind) { mind = d; mindSegment = sn;}
            }
        }
    }
    else //open curve
...
void ScanlineFill(int c , int mycol, QImage *Thresh, struct PointList *LPointList)
{
    ...
    {
        //use spline colours
        red = (uchar) (cv->Colour[0]);  g = (uchar) (cv->Colour[1]);  b= (uchar) (cv->Colour[2]);
    }
    else if (mycol == 0) //seg colours
    {	
    	red= (Segments[(cv->Segment)-1]->Colour[0]);    g= (Segments[(cv->Segment)-1]->Colour[1]);   b= (uchar) (Segments[(cv->Segment)-1]->Colour[2]);
    }
            ...
            for (m = qRound(intersections[0]); m<fwidth; m++)
            {
                ...
                if (fill)
                {
                	if (m>=0) //watch for 'off left' errors
	                {
	                    ...
	                    {
	                        //use spline colours
	                        RED(data,apos) = red; GREEN(data,apos) = g; BLUE(data,apos) = b;
	                    }
	                    else  //mask colours
	                    {
...
void DrawCurveOutput(int c, int file, uchar *data, QList <bool> *UseMasks, bool remove)
{
   ...
   uchar setval;
   if (remove) setval=0; else setval=255;
...
//Direct translation from old VB sub
   ...
   file *=zsparsity;
    ...
    {
        //use spline colours
        r = (uchar) (cv->Colour[0]);  g = (uchar) (cv->Colour[1]);  b= (uchar) (cv->Colour[2]);
    }
    else if (mycol == 0) //seg colours //seg mode or similar
    {	
    	r= (Segments[(cv->Segment)-1]->Colour[0]);    g= (Segments[(cv->Segment)-1]->Colour[1]);   b= (uchar) (Segments[(cv->Segment)-1]->Colour[2]);
    }
               ...
               for (sm = 1; sm<=d; sm++)
               {
                   ...
                   if (sy >= 0 && sy < fheight && sx >= 0 && sx < fwidth)
                   {
                       ...
                       if (mycol < 2  || cv->Segment==0)
                       {
                           ...
                           RED(data,apos) = r; GREEN(data,apos) = g; BLUE(data,apos) = b;
                       }
                       else  //mask colours
                       {
...
void DrawCurveMarkers(QGraphicsScene *scene)
{
	...
	for (int i=0; i<p->Count; i++)
	{
		if (CurveMarkersAsCrosses)
		{
			...
			if (i>=p->Count-2) 
				{newitem1->setPen(redpen); newitem3->setPen(redpen); } 
			else 
				{newitem1->setPen(mypen); newitem3->setPen(mypen); }
		...
		else //plain circle markers
		{
			...
			else
			{
				...
				if (i==p->Count-2) newitem->setPen(redpen); else newitem->setPen(mypen);
				newitem->setZValue(2);
				MarkerList.append((QGraphicsItem *) newitem);
				scene->addItem(newitem);
			}
...
void WorkOutNewNodes(int scurve, int filenum, double SplineLength, QList <double> *NodeSpacings, int NodesToUse, QList <double> *NodeX, QList <double> *NodeY, QList <double> *NodeNormX, QList <double> *NodeNormY, int ArrayBase)
{
    ...
    if (Curves[scurve]->Closed)
    {
        ...
        for (i = 0; i<NodesToUse; i++)
        {
        	if (currentpos < (*NodeSpacings)[nextpoint])
            {
                ...
                ratiopos2 = ratiopos + 0.01;       if (ratiopos2 > 1) ratiopos2 = 1;
...
{
    Index++;
    if (Index>=(stretches->size()-1)) if (Index==(stretches->size()-1)) return (*stretches)[Index]; else return 0;
...
void SurfaceCurve(int scurve, int firstfile, int lastfile, QList <double> *stretches, int resamps, QVector <double> *TrigArray, int *TrigCount)
{
	...
	QList <double> NodeSpacings; for (i=0; i<max; i++) NodeSpacings.append(0); //initialise to 0's
