...
Cache::Cache() 
{ 
	Filenum=-1; ColData=0;Locks=0;Masks=0; 
	GreyData.clear(); 	
	ColDataCompressed=0;
	for (int i=0; i<SegmentCount; i++)
...
void ClearCache() //use after deleting a segment and similar circs
{
	...
	qDeleteAll(Caches.begin(), Caches.end()); Caches.clear();
	mutex.unlock();
}
...
int CacheIndex(int fnum)
{
	...
	for (int i=0; i<Caches.count(); i++)
	{	
		if (Caches[i]->Filenum==fnum) {Caches[i]->LastUsed = QDateTime::currentDateTime(); return i;}
	}return -1; //not found
}
	...
	{		
		...
		for (i=0; i<Caches.count(); i++)
		{
			if (Caches[i]->Filenum==-1) //empty
				{Caches[i]->Blank(fnum); return i;}
			if (Caches[i]->LastUsed<=oldest) {oldest=Caches[i]->LastUsed; bestindex=i;}
		}
...
//		qDebug()<<"Didn't find cache entry, loading from disk";
	...
	if (ColArray.format()==QImage::Format_Indexed8) GreyImage=true; else GreyImage=false;
	cwidth4=cwidth;
...
//Get info from segment file
{
	...
	sfname.append(temp); 
        ...
        char tbuff[5];
        if (QFile(sfname+".bmp").exists()){ sfname+=".bmp"; tbuff[0]='B';tbuff[1]='M';tbuff[2]='P';tbuff[3]=0;}
        else if (QFile(sfname+".png").exists()) {sfname+=".png"; tbuff[0]='B';tbuff[1]='M';tbuff[2]='P';tbuff[3]=0;}
	//Load it from disk
...
void SimpleSaveGreyData(int fnum, int seg, QImage greydata)
{
	...
	{greydata.save(sfnamebmp); QFile(sfnamepng).remove();}
	else 
	{greydata.save(sfnamepng); QFile(sfnamebmp).remove();}	
}
...
void SaveGreyData(int fnum, int seg)
{
	...
	//now write the data - autoguesses format, so this should do it!
	if (FileCompressionLevel==0) 
	{GA[seg]->save(sfnamebmp); QFile(sfnamepng).remove();}
	else 
	{GA[seg]->save(sfnamepng); QFile(sfnamebmp).remove();}
	...
	if (CacheCompressionLevel==0)
	{
		...
		if (Caches[newcache]->GreyData[seg]->Data==0) {Caches[newcache]->GreyData[seg]->Data = GA[seg]; }
...
//Save the masks as a byte array
{
	...
	if (CacheCompressionLevel>0 && CacheCompressionLevel!=FileCompressionLevel)
	{
...
void ApplyDefaultSettings()
{
	...
	qDeleteAll(Segments.begin(), Segments.end());OutputObjects.clear();
	qDeleteAll(MasksSettings.begin(), MasksSettings.end());MasksSettings.clear();
	qDeleteAll(Curves.begin(), Curves.end());Curves.clear();
	qDeleteAll(OutputObjects.begin(), OutputObjects.end());OutputObjects.clear();
	qDeleteAll(UndoEvents.begin(), UndoEvents.end()); UndoEvents.clear();
	qDeleteAll(RedoEvents.begin(), RedoEvents.end()); RedoEvents.clear();
	TotalUndoSize=0;
	Segments.clear();
	MasksSettings.clear();
	Curves.clear();
	...
	CMin=0;
	Trans=0;
	if (QThread::idealThreadCount ()>1) BackgroundCacheFilling=true; else BackgroundCacheFilling=false;
	SegmentCount=1; Segments.append(new Segment("Default"));
	...
	PixPerMM=1; SlicePerMM=1; SkewDown=0; SkewLeft=0;
	FirstOutputFile=0; LastOutputFile=FileCount-1;
	MaxTriangles=4000;
	RangeHardFill=false;
	RangeSelectedOnly=false;
	OutputMirroring=false;
	HiddenMasksLockedForGeneration=false; SegmentBrushAppliesMasks=false;
	CurveShapeLocked=false;
	SegmentsLocked=false; CurveMarkersAsCrosses=false; CurveShapeLocked=false;
	PixSens=1; XYDownsample=1; ZDownsample=1;
...
int GetShort(QDataStream *in)
{
    ...
    * in >> val;           
    Counter+=2;
	if (in->status()) {qDebug()<< "getshort"<<in->status(); Errors++;}
	return (int) val;
}
...
int GetInt(QDataStream *in)
{
	 ...
	 *in >> val;           // extract int - hope endianness is correct! 
     Counter+=4;
     if (in->status()) {qDebug()<< "getint"<<in->status(); Errors++;}
  	 return val;	
}
...
int GetBool(QDataStream *in)
{
	 ...
	 *in >> val;           // extract int - hope endianness is correct! 
    Counter+=2;
  	 if (in->status()) {qDebug()<< "getbool"<<in->status(); Errors++;}
...
int GetVariantInt(QDataStream *in)
{
	int test;
	test=GetShort(in);
	if (test==0) { 	if (in->status()) Errors++; return 0;} 
		 else {test=GetShort(in);  	if (in->status()) {qDebug()<< "getvariantint"<<in->status(); Errors++;}; return test;}
}
...
double GetDouble(QDataStream *in)
{
	...
	*in >> test;
	Counter+=8;
	if (in->status()) {qDebug()<< "getdouble"<<in->status(); Errors++;};
	return test;	
}
...
void GetBytes(QDataStream *in, char * ptr, int bytes)
{
	...
	Counter+=bytes;
	if (in->status()) {qDebug()<< "getbytes"<<in->status(); Errors++;};
	return;
}
	 ...
	 
{
	...
	*/
	if (in->status()) {qDebug()<< "getstring"<<in->status(); Errors++;};
	return text;
}


void OldGetSettings(QDataStream *in,int tempint) {Error("Import from this version of SPIERS not implemented - convert to most recent VB version, then try again");} //pass initial value
void GetSettingsMinus1(QDataStream *in) {Error("Import from this version of SPIERS (file version 1) not implemented - convert to most recent VB version, then try again");}
void GetSettingsMinus2(QDataStream *in) {Error("Import from this version of SPIERS (file version 2) not implemented - convert to most recent VB version, then try again");}
void GetSettingsMinus3(QDataStream *in) {Error("Import from this version of SPIERS (file version 3) not implemented at all, anywhere, sorry! I hope you never see this message...");}
void GetSettingsMinus4(QDataStream *in) {Error("Import from this version of SPIERS (file version 4) not implemented - convert to most recent VB version, then try again");}
void GetSettingsMinus5(QDataStream *in) {Error("Import from this version of SPIERS (file version 5) not implemented - convert to most recent VB version, then try again");}
...
void GetSettingsMinus6(QDataStream *in) 
{
	...
	for (n=0; n<SegmentCount; n++) 
		{sstring.sprintf("Segment %d",n+1); Segments.append(new Segment(sstring));}
	...
	for (n=0; n<8; n++) {itemp=GetShort(in); if (n<SegmentCount) Segments[n]->LinGlobal = itemp;}
	for (n=0; n<8; n++) {temp=GetBool(in); if (n<SegmentCount) Segments[n]->LinInvert = temp;}
	for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->NeighbourBright = dtemp;}
	for (n=0; n<8; n++) {itemp=GetInt(in); if (n<SegmentCount) Segments[n]->NeighbourSparse = itemp;}
	for (n=0; n<8; n++) {temp=GetBool(in); if (n<SegmentCount) Segments[n]->NeighbourSingle = temp;}
	...
	for (m=0; m<10; m++) for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->PolyRedConsts[m] = dtemp;}
	for (m=0; m<10; m++) for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->PolyGreenConsts[m] = dtemp;}
	for (m=0; m<10; m++) for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->PolyBlueConsts[m] = dtemp;}
	for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->PolyKall = dtemp;}
	for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->PolyOrder = (int)dtemp;}
	for (n=0; n<8; n++) {itemp=GetShort(in); if (n<SegmentCount) Segments[n]->PolySparse = itemp;}
	for (n=0; n<8; n++) {itemp=GetShort(in); if (n<SegmentCount) Segments[n]->PolyRetries = itemp;}
	for (n=0; n<8; n++) {itemp=GetShort(in); if (n<SegmentCount) Segments[n]->PolyConverge = itemp;}
	for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->PolyScale = dtemp;}
	for (n=0; n<8; n++) {itemp=GetShort(in); if (n<SegmentCount) Segments[n]->PolyContrast = itemp;}
	...
	for (n=0; n<CurveCount; n++) 
		{sstring.sprintf("Curve %d",n+1); Curves.append(new Curve(sstring));}
	...
	for (m=0; m<3; m++) for (n=0; n<255; n++) //There are 255 not 256 curves
		{itemp=GetInt(in); if (n<CurveCount) Curves[n]->Colour[m]=itemp;}
	for (n=0; n<255; n++) //There are 255 not 256 curves
		{temp=GetBool(in); if (n<CurveCount) Curves[n]->Closed=temp;}		
	for (n=0; n<255; n++) //There are 255 not 256 curves
		{temp=GetBool(in); if (n<CurveCount) Curves[n]->Filled=temp;}		
	for (n=0; n<255; n++) //There are 255 not 256 curves
		{itemp=GetShort(in); if (n<CurveCount) Curves[n]->Segment=itemp;}
	...
	OutputObjects.clear();OutputObjectsCount=0;
...
*/
	...
	for (m=0; m<3; m++) for (n=0; n<201; n++) 
		{itemp=GetShort(in); if (n<OutputObjectsCount) OutputObjects[n]->Colour[m]=itemp;}
	...
	MaxTriangles=GetInt(in); if(MaxTriangles < 5) MaxTriangles = 2000;
	...
	for (n=0; n<8; n++) {itemp=GetShort(in); if (n<SegmentCount) Segments[n]->RangeBase = itemp;}
	for (n=0; n<8; n++) {itemp=GetShort(in); if (n<SegmentCount) Segments[n]->RangeTop = itemp;}
	for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->RangeCenter = dtemp;}
	for (n=0; n<8; n++) {dtemp=GetDouble(in); if (n<SegmentCount) Segments[n]->RangeGradient = dtemp;}
...
//  	FullFiles=Files; //back it up
	...
	if (Errors>0) {
		QString Mess;
		Mess.sprintf("Warning - %d read errors encountered in settings file",Errors);Message(Mess);DumpSettings();}
...
//This is old Vb import one
{
    ...
    QFile file(Fname);
    if (!file.open(QIODevice::ReadOnly)) {Message("No settings file - using defaults"); return false;}
    ...
    {
    case -1:
        GetSettingsMinus1(&in); break;
    case -2:
        GetSettingsMinus2(&in); break;
    case -3:
        Error("ERROR - file version 3 not supported"); break;
    case -4:
        GetSettingsMinus4(&in); break;
    case -5:
        GetSettingsMinus5(&in); break;
    case -6:
        GetSettingsMinus6(&in); break;
    default:
    	QString temp;
    	temp.sprintf("Settings file is version %d - too recent for this version of SPIERSedit, sorry!",tempint); Error(temp);
    }
    file.close();
	return true;
}
...
//Write Settings File
{
    ...
    QFile file(sfname);
    if (!file.open(QIODevice::WriteOnly)) {Message("Warning - can't open settings file " + SettingsFileName + " to write to"); return;}
...
//Read Settings File - QT version (i.e. the spe file
{
    ...
    if (!file.open(QIODevice::ReadOnly)) {Message("Warning - can't open settings file " + SettingsFileName + ".spe to read from"); return;}
	QDataStream in(&file);
	...
	in>>dummy;
	if (dummy!="SPIERSedit settings file") {Message("Warning - settings file " + SettingsFileName + ".spe is not a valid SPIERSedit file"); return;}
...
//		qDebug()<<"RS4b-in loop 3"<<i;
	...
	for (i=0; i<OutputObjectsCount; i++) 
	{
		...
		o->ComponentMasks.clear(); o->ComponentSegments.clear(); o->CurveComponents.clear();
		int cmcount;
		int cscount;
		int cccount;
		in >> cmcount;		
		in >> cscount;
		in >> cccount;	
		for (n=0; n<cmcount; n++) {in>>dummy_int; o->ComponentMasks.append(dummy_int);}
		for (n=0; n<cscount; n++) {in>>dummy_int; o->ComponentSegments.append(dummy_int);}
		for (n=0; n<cccount; n++) {in>>dummy_int; o->CurveComponents.append(dummy_int);}
		in >> o->ListOrder;
		in >> o->IsGroup;
		in >> o->Parent; 
		in >> o->Key;
	...
	if (!in.atEnd()) in >> ShowSlicePosition;
 	...
 	if (!in.atEnd()) for (int i=0; i<FileCount; i++) 
 		{in>>dummy_double; FullStretches.append(dummy_double);}
