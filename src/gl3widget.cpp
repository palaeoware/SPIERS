...
//    #endif
    ...
    for (int charnumber=16; charnumber<26; charnumber++)
    {
        ...
        {
            fontVertices <<QVector3D(((float)(rowmans[charnumber][i*4]))/FONT_SCALE_FACTOR,((float)(rowmans[charnumber][i*4+1]))/FONT_SCALE_FACTOR,0.0) << QVector3D(((float)(rowmans[charnumber][i*4+2]))/FONT_SCALE_FACTOR,((float)(rowmans[charnumber][i*4+3]))/FONT_SCALE_FACTOR,0.0);
        }
    ...
    for (int ii=10; ii<13; ii++)
    {
        ...
        VBOcharacters[ii].allocate(6*sizeof(GLfloat)*(rowmans_size[charnumber]/4));
        VBOcharacters[ii].write(0,fontVertices.constData(),6*sizeof(GLfloat)*(rowmans_size[charnumber]/4));
    ...
    occVertices << QVector3D(-500000, -50000,  -1.001) << QVector3D( 500000, -500000,  -1.001) << QVector3D( 500000,  500000,  -1.001) // Front
                 << QVector3D( 500000,  50000,  -1.001) << QVector3D(-500000,  500000,  -1.001) << QVector3D(-500000, -500000,  -1.001);
    occNormals << QVector3D( 0,  0,  1) << QVector3D( 0,  0,  1) << QVector3D( 0,  0,  1) // Front
    ...
    occBuffer.create();
    occBuffer.bind();
    occBuffer.allocate(numOccVertices * (3 + 3 + 2) * sizeof(GLfloat));

    int offset = 0;
    occBuffer.write(offset, occVertices.constData(), numOccVertices * 3 * sizeof(GLfloat));
    offset += numOccVertices * 3 * sizeof(GLfloat);
    occBuffer.write(offset, occNormals.constData(), numOccVertices * 3 * sizeof(GLfloat));
...
void GlWidget::DoPMatrix(int width,int height)
{
    ...
    float fudge=(float)1300/(float)width;
    ...
    if (MainWin->ui->actionOrthographic_View->isChecked())
        ...
        pMatrix.perspective(ClipAngle, (float) width / (float) height, ClipStart,ClipDepth);
}
...
void GlWidget::DrawLine(QMatrix4x4 vMatrix, QVector3D lPosition,float pos, bool major, bool horizontal)
{
    ...
    if (!horizontal) vMatrix.translate(pos,0,-1); else vMatrix.translate(0,pos,-1);
...
void GlWidget::RenderCharacter(GLfloat x, GLfloat y, GLfloat z, int charactercode, QMatrix4x4 vMatrix, QColor Colour)
{
    ...
    vMatrix.scale((ClipAngle / (float)this->height()));
...
//mm true if size in mm - else render m
{
    ...
    //add int part
    QString inumstring;
    inumstring.sprintf("%d",(int)number);
    ...
    if (decimalplaces>0)
    {
        ...
        number-=(float((int)number)); //get decimal part
        number *= pow(10.0,float(decimalplaces));

        inumstring.sprintf("%d",(int)(number+.5));
            for (int i=0; i<inumstring.length();i++)
                characters.append(inumstring.toLatin1().at(i)-'0');
    }
    ...
    for (int i=0; i<characters.length(); i++)
    {
        ...
        off+=CharacterWidths[characters[i]]*(((0.9/FONT_SCALE_FACTOR)*ClipAngle / (float)this->height()));
    }
...
void GlWidget::DrawScaleGrid(QMatrix4x4 vMatrix, QVector3D lPosition)
{
    ...
    if (dp<0) dp=0;
    bool mm=true;
    for (int i=-10; i<10; i++) RenderNumber((((10.0/FONT_SCALE_FACTOR)*ClipAngle / (float)this->height())),((float)i)*s*coarse - (((10.0/FONT_SCALE_FACTOR)*ClipAngle / (float)this->height())),-.99,((float)i*coarse),dp,mm,true,vMatrix);
    for (int i=-10; i<10; i++) if (i!=0) RenderNumber((float)i*s*coarse +(((10.0/FONT_SCALE_FACTOR)*ClipAngle / (float)this->height())),0-(((10.0/FONT_SCALE_FACTOR)*ClipAngle / (float)this->height())),-.99,((float)i*coarse),dp,mm,true,vMatrix);
}

void GlWidget::DrawObjects(bool rightview, bool halfsize)
{
    //qDebug()<\<"In DrawObjects widget";
    QMatrix4x4 vMatrix; //view matrix
    vMatrix.setToIdentity();


    QVector3D cameraPosition = QVector3D(0, 0, campos);
    QVector3D rightcameraPosition= QVector3D(StereoSeparation*STEREO_SEPARATION_MODIFIER/campos, 0, campos);
    QVector3D cameraUpDirection = QVector3D(0, 1, 0);

    //old code for this - didn't work
          //if (rightview)
          //      vMatrix.lookAt(cameraPosition, QVector3D(StereoSeparation*3.0/campos, 0, 0), cameraUpDirection); // first 0 is StereoSeparation*3.0/campos
          //else
           ...
           if ((SVObjects[i]->Transparency==0 && trans==0) || (SVObjects[i]->Transparency!=0 && trans==1)) //do trans on second run
           {
            ...
            if (MainWin->ui->actionBounding_Box->isChecked())
            {
                if (SVObjects[i]->BoundingBoxBuffer.isCreated())
                {
                    ...
                    useshader->setUniformValue("alpha",(GLfloat) 1.0);
            ...
            else
            {
                ...
                useshader->setUniformValue("shininess", (GLfloat) (200.0-pow((double)180.0,shininess)));
...
{
    //qDebug()<<"paintgl";
    glfunctions->glClearColor((float)back_red/(float)255, (float)back_green/(float)255, (float)back_blue/(float)255, 0.5f);
