...
//Constructor for a segment - set defaults
{
	...
	Colour[2]= 128 + int( (double)127 * (qrand()/RAND_MAX)); 
        LinPercent[0]=100; LinPercent[1]=100; LinPercent[2]=100; LinGlobal=100;
	LinInvert=false;
	...
	PolyKall;*/
	PolyOrder=4;
	PolySparse=1; PolyRetries=10; PolyConverge=3;
...
OutputObject::OutputObject(QString name)
{
	...
	Resample=100;
	Colour[0]=255; Colour[1]=255; Colour[2]=255; //r,g,b
...
void OutputObject::SetUpForRender()
{
     ...
     UseMasks.clear();
     for (int i=0; i<=MaxUsedMask; i++) if ((ComponentMasks.indexOf(i))>=0) UseMasks.append(true); else UseMasks.append(false);

     UseSegs.clear();
     for (int i=0; i<SegmentCount; i++) if ((ComponentSegments.indexOf(i))>=0) UseSegs.append(true); else UseSegs.append(false);
}
...
void RecentFile(QString fname)
{
	...
	{
		if (RecentFileList[n].File==fname) {RecentFileList.removeAt(n); break;}
	}
