...
void MainWindowImpl::on_actionExport_Curves_as_CSV_triggered()
{
    ...
    QFile file(filen);
    if (!file.open(QIODevice::WriteOnly)) {Message("Warning - can't open"); return;}
...
void MainWindowImpl::on_actionImport_Curves_as_CSV_triggered()
{
    ...
    QFile file(filen);
    if (!file.open(QIODevice::ReadOnly)) {Message("Warning - can't open"); return;}
...
void MainWindowImpl::on_actionOutput_visible_image_set_triggered()
{
    ...
    if (ExportingImages==true) {ExportingImages=false; return;}
