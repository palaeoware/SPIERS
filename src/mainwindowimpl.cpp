	...
	: QMainWindow(parent, f)
{
	...
	QAction *shortcutq = new QAction("q_pressed", this); shortcutq->setShortcut(tr("q"));
	QAction *shortcuta = new QAction("q_pressed", this); shortcuta->setShortcut(tr("a"));
	shortcutright  = new QAction("right_pressed", this); shortcutright->setShortcut(tr("."));
	shortcutleft = new QAction("left_pressed", this); shortcutleft->setShortcut(tr(","));
        shortcutright2  = new QAction("right_pressed", this); shortcutright2->setShortcut(tr("ctrl+."));
        shortcutleft2 = new QAction("left_pressed", this); shortcutleft2->setShortcut(tr("ctrl+,"));
        shortcutspace = new QAction("space_pressed", this); shortcutspace->setShortcut(tr("space"));
...
void MainWindowImpl::autosave() {	if (pausetimers) return; if (Active) {WriteSettings();}}
...
void MainWindowImpl::Undo()
{
	...
	if (Active)
	{
        ...
        if (proceed==true)
        {
            ...
            if (UndoEvents.count()>1) actionUndo->setEnabled(true); else actionUndo->setEnabled(false);
        }
	}
	if (RedoEvents.count()>0) actionRedo->setEnabled(true); else actionRedo->setEnabled(false);
}
...
void MainWindowImpl::SaveSettings() {WriteAllData(CurrentFile);	WriteSettings(); WriteSuperGlobals(); Message("Settings saved");}
...
void MainWindowImpl::ScreenUpdate()
{
	...
	if (Active)
	{
		...
		MoveFlag=false; ChangeFlag=false; //reset flags
	}
	else
	{
		...
		if (openfile.length()>=2)
		{
			...
			QFile f(openfile);
			if (f.exists()==false) {Message("File "+openfile+" does not exist"); return;}
...
void MainWindowImpl::q_pressed() {ZoomSlider->setValue(ZoomSlider->value()+20);}
void MainWindowImpl::a_pressed() {ZoomSlider->setValue(ZoomSlider->value()-20);}
void MainWindowImpl::right_pressed() {SliderPos->setValue(CurrentFile+2);}
void MainWindowImpl::left_pressed() {SliderPos->setValue(CurrentFile);}
...
{
	if (actionShowMasks->isChecked()) MasksFlag=true; else MasksFlag=false;	
	ShowImage(graphicsView);
}

void MainWindowImpl::SetSegsFlag()
{
	if (actionShowSegs->isChecked()) SegsFlag=true; else SegsFlag=false;	
	ShowImage(graphicsView);
}
...
//Transparency setting changed - set global, redraw
{
	...
	if (ThreeDmode)
	{		
		if (SquareBrush) Brush.resize(Brush_Size,2,0); else Brush.resize(Brush_Size,3,0);
		if (LastMouseX>0) Brush.draw(LastMouseX, LastMouseY);
	}
	else
	{
		if (SquareBrush) Brush.resize(Brush_Size,0,0); else Brush.resize(Brush_Size,1,0);
		if (LastMouseX>0) Brush.draw(LastMouseX, LastMouseY);
	}
}
...
void MainWindowImpl::Preset1() {BrushSize->setValue(1);}
void MainWindowImpl::Preset2() {BrushSize->setValue(2);}
void MainWindowImpl::Preset3() {BrushSize->setValue(3);}
void MainWindowImpl::Preset4() {BrushSize->setValue(4);}
void MainWindowImpl::Preset5() {BrushSize->setValue(5);}
void MainWindowImpl::Preset6() {BrushSize->setValue(8);}
void MainWindowImpl::Preset7() {BrushSize->setValue(15);}
void MainWindowImpl::Preset8() {BrushSize->setValue(30);}
void MainWindowImpl::Preset9() {BrushSize->setValue(50);}
void MainWindowImpl::Preset0() {BrushSize->setValue(200);}
void MainWindowImpl::cmac() 
{
	if (actionCurve_markers_as_crosses->isChecked()) CurveMarkersAsCrosses=true; else CurveMarkersAsCrosses=false;
	ShowImage(graphicsView);
}
void MainWindowImpl::LockShape() {if (actionLock_curve_shape->isChecked()) CurveShapeLocked=true; else CurveShapeLocked=false;}
...
//Zoom slider changed - convert to real zoom, set zoom box
{
    ...
    CurrentZoom=(pow(10.0,((double)(zoom))/500+1))/100;
    DontRedoZoom=true;
...
void MainWindowImpl::on_ZoomSpinBox_valueChanged(int zoom)
{
    ...
    double t=log10((double)zoom); //t=(slider/500)+1
    int slider = (t-1)*500;
    CurrentZoom=(pow(10.0,((double)(slider))/500+1))/100;
    DontRedoZoom=true;
    ZoomSpinBox->setValue((int)(CurrentZoom*100));
...
{
	CMin=val;
	if (MaxSlider->value()<(val)) {MaxSlider->setValue(val);	MaxSlider->update();}
	ShowImage(graphicsView);	
}
...
{
	CMax=val;
	if (MinSlider->value()>val) {MinSlider->setValue(val);	MinSlider->update();}
	ShowImage(graphicsView);	
}
...
void MainWindowImpl::TransToggled()
{
	...
	if (Trans<15) {LastTrans=Trans; Trans=15;} else Trans=LastTrans;
	TransSlider->setValue(Trans);
}

void MainWindowImpl::ThresholdFlag()
{
	if (actionThreshold->isChecked()) ThreshFlag=true; else ThreshFlag=false;
	ShowImage(graphicsView);
}
...
//Makes a copy
{
	...
	if (file.contains(".") || file.contains("/") || file.contains("\\")) 
		{Message("Illegal Filename - try again"); return;}
    ...
    {    			
		Message("ERROR - can't create SPIERSedit settings file " + file + ".spe - filename probably invalid"); return;
	}

	testfile.close(); testfile.remove(); //delete test version
	...
	WriteSettings(); //have to save current one first...
	 QDir fromdir(olddirname); QDir todir(newdirname); QDir rootdir(rootdirname);
...
void MainWindowImpl::Menu_File_Import()
{
	...
	*/
	...
	if (!(settings.exists()))
	{
		Message("ERROR - couldn't find a SPIERS 1.1 dataset here (imed/settings.dat missing)"); return;
	}
	...
	//looks OK	
	settings2.close(); settings2.remove(); //delete test version
	FullSettingsFileName=Fname.left(lastsep)+"/"+impdialog.fname+".spe";
	SettingsFileName=impdialog.fname; FileNotes=impdialog.notes; //get globals in
	
	setname=Fname.left(lastsep) + "/imed"; 	QDir fromdir(setname);
	setname=Fname.left(lastsep) + "/" + impdialog.fname; 	QDir todir(setname); 
	...
	CopyingImpl cop; 	cop.Copy(fromdir,todir); //move files, import the curves 
		...
		if (item==MasksSettings[i]->widgetitem) //found it
		{
			...
			if (column==3) //foreground
			{
				...
				if (newcol.isValid())
				{
					...
					int c=MasksSettings[i]->Contrast+1; if (c==6) c=20000; //big value, want 0's!
...
{
	if (MaxUsedMask==0) {Message("Can't delete the last mask!"); return;}
	QList <int> list;
	for (int i=0;  i<=MaxUsedMask; i++)
	{
		if ((MasksSettings[i]->widgetitem)>0)
...
void MainWindowImpl::on_actionLock_selected_segments_triggered()
{
	for (int i=0;  i<SegmentCount; i++)
	{
		if ((Segments[i]->widgetitem)>0) if ((Segments[i]->widgetitem)->isSelected())
			{ Segments[i]->Locked=true; RefreshOneSegmentItem(Segments[i]->widgetitem,i);	}	
	}
}
...
void MainWindowImpl::on_actionUnlock_selected_segments_triggered()
{
	...
	{
		if ((Segments[i]->widgetitem)>0) if ((Segments[i]->widgetitem)->isSelected())
			{ Segments[i]->Locked=false; RefreshOneSegmentItem(Segments[i]->widgetitem,i);	}	
	}
}
...
void MainWindowImpl::on_actionActivate_selected_segments_triggered()
{
	...
	{
		if ((Segments[i]->widgetitem)>0) if ((Segments[i]->widgetitem)->isSelected())
			{ Segments[i]->Activated=true; RefreshOneSegmentItem(Segments[i]->widgetitem,i);	}	
	}
	ShowImage(graphicsView);
}
...
void MainWindowImpl::on_actionDeactivate_selected_segments_triggered()
{
	...
	{
		if ((Segments[i]->widgetitem)>0) if ((Segments[i]->widgetitem)->isSelected())
			{ Segments[i]->Activated=false; RefreshOneSegmentItem(Segments[i]->widgetitem,i);	}	
	}
	ShowImage(graphicsView);
}
...
void MainWindowImpl::on_actionNew_mask_triggered() {on_MaskAdd_pressed();}
void MainWindowImpl::on_actionDelete_selected_mask_s_triggered() {on_MaskDelete_pressed();}
void MainWindowImpl::on_actionCreate_new_segment_triggered() {on_SegmentAdd_pressed();}
void MainWindowImpl::on_actionDelete_selected_segments_triggered() {on_SegmentDelete_pressed();}
...
void MainWindowImpl::on_actionCurve_markers_as_crosses_triggered() {cmac();}
...
{
	if (actionSegment_brush_applies_masks->isChecked()) SegmentBrushAppliesMasks=true; else SegmentBrushAppliesMasks=false;
}
...
{
	if (index==2) GenInvert->setVisible(false); else GenInvert->setVisible(true); 
}
