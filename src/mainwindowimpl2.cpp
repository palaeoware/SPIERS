...
//		if (MasksSettings[i]->Write) write->setPixmap(QPixmap(":/icons/disk.bmp"));
//		else write->setPixmap(QPixmap(":/icons/disk_off.bmp"));
//		MasksTreeWidget->setItemWidget (item, 4, write);
...
void MainWindowImpl::RefreshMasks()
{
	...
	//find lowest
		...
		QStringList strings;
		strings.append("");		strings.append(""); strings.append("");
		strings.append("");
		strings.append("");		strings.append("");		strings.append("");		
		QString m;
		strings.append(m);
...
bool MainWindowImpl::AmIMerged(int i)
{
	...
	if (OutputObjects[i]->Parent==-1) return false; else return AmIMerged(OutputObjects[i]->Parent);
}
...
void MainWindowImpl::OODrawChildren( QList <bool> selflags, int parent)
{
	...
	//first find lowest listorder...
	QList <bool> usedflags; for (int i=0; i<OutputObjectsCount; i++) usedflags.append(false);
	...
	//find lowest
		...
		for (int j=0; j<OutputObjectsCount; j++)
		{
			if (OutputObjects[j]->ListOrder<lowestval && usedflags[j]==false && OutputObjects[j]->Parent==parent) 
				{lowestval=OutputObjects[j]->ListOrder; lowestindex=j;}
		}
		int i=lowestindex;
...
void MainWindowImpl::RefreshOO()
{
	...
	if (OutputObjectsCount==0) {OOTreeWidget->clear(); return;}
	...
	for (int i=0; i<OutputObjectsCount; i++)
	{
		...
		if (OutputObjects[i]->widgetitem)
		{
			...
			if ((OutputObjects[i]->widgetitem)->isExpanded()) OutputObjects[i]->Expanded=true; else OutputObjects[i]->Expanded=false;
		}	
		selflags.append(sf);
	}	
	OOTreeWidget->clear();
...
void MainWindowImpl::RefreshCurves()
{
	...
	//qDebug()<<"RC4";
	QList <bool> usedflags; for (int i=0; i<CurveCount; i++) usedflags.append(false);
	...
	//find lowest
		...
		for (int j=0; j<CurveCount; j++)
		{
			if (Curves[j]->ListOrder<lowestval && usedflags[j]==false) 
				{lowestval=Curves[j]->ListOrder; lowestindex=j;}
		}
		int i=lowestindex;
		usedflags[i]=true;

                QStringList strings; for (int j=0; j<5; j++) strings.append("");
                item=new QTreeWidgetItem(strings);
...
void MainWindowImpl::RefreshSegmentsBoxes() 
{
	...
	if (found==-1) SegBoxLeft->setCurrentIndex(0); else SegBoxLeft->setCurrentIndex(found);

	found=SegBoxRight->findData(CurrentRSegment); 
	if (found==-1) SegBoxRight->setCurrentIndex(0); else SegBoxRight->setCurrentIndex(found);
...
void MainWindowImpl::RefreshMasksBoxes()
{
	...
	if (found==-1) MaskBoxLeft->setCurrentIndex(0); else MaskBoxLeft->setCurrentIndex(found);

	found=MaskBoxLeft->findData(SelectedRMask); 
	if (found==-1) MaskBoxRight->setCurrentIndex(0); else MaskBoxRight->setCurrentIndex(found);
	clearing=false;
}
...
//	MaxTrianglesSB->setValue(MaxTriangles);
	...
	//qDebug()<<"MG3";
	SliceSelector->setVisible(MenuSliceSelectorChecked);  actionSlice_Selector->setChecked(MenuSliceSelectorChecked);
	DockMasksSettings->setVisible(MenuMasksChecked); actionMasks->setChecked(MenuMasksChecked);
	DockCurvesSettings->setVisible(MenuCurvesChecked); actionCurves->setChecked(MenuCurvesChecked);
	DockSegmentsSettings->setVisible(MenuSegsChecked); actionSegments->setChecked(MenuSegsChecked);
	dockWidget_Generate->setVisible(MenuGenChecked); actionGeneration->setChecked(MenuGenChecked);
	DockOutputSettings->setVisible(MenuOutputChecked); actionOutput->setChecked(MenuOutputChecked);
	dockWidget_Main->setVisible(MenuToolboxChecked); actionMain_Toolbox->setChecked(MenuToolboxChecked);
	DockHist->setVisible(MenuHistChecked); actionHistorgram->setChecked(MenuHistChecked);
	DockInfo->setVisible(MenuInfoChecked); actionHistorgram->setChecked(MenuInfoChecked);
	...
	{	
        ...
        if ((i+1) % 50 == 0){ item->setTextColor(QColor("dark blue"));}
        if ((i+1) % 100 == 0){ item->setTextColor(QColor("dark red")); }
...
void MainWindowImpl::on_SegmentAdd_pressed()
{
	...
	Segments.append(m);
	QStringList strings;
	strings.append("");		strings.append("");  strings.append(""); strings.append("");	strings.append("");	
...
void MainWindowImpl::on_SegmentDelete_pressed()
{
	...
	if (list.count()==SegmentCount) {Message("Can't delete the last segment"); return;}
	if (list.count()>0)
	{
		QString mess="";
...
void MainWindowImpl::on_CurvesTreeWidget_itemDoubleClicked(QTreeWidgetItem* item, int column)
{
	...
	for (int i=0; i<CurveCount; i++)
		if (item==Curves[i]->widgetitem) //found it
		{
			if (column==2) //widget 
			{
				if (Curves[i]->Closed==false) {Curves[i]->Closed=true; Curves[i]->Filled=false;}
				else if (Curves[i]->Filled==false) {Curves[i]->Filled=true;}
				else {Curves[i]->Closed=false; Curves[i]->Filled=false;}
...
void MainWindowImpl::on_SliceSelectorList_itemSelectionChanged()
{
	...
	for (int i=0; i<FileCount; i++)
	{
		...
		QFont f=t->font();
		if (t->isSelected())	f.setUnderline(true); else 	f.setUnderline(false);
		t->setFont(f);
	}
}
...
void MainWindowImpl::on_CurvesTreeWidget_itemSelectionChanged()
{
	...
	//qDebug()<<"ISC3";
	if (selitems.count()!=1) SelectedCurve=-1; else
	{
		for (int i=0; i<CurveCount; i++)
...
void MainWindowImpl::on_Curve_Add_pressed()
{
	...
	Curves.append(c);
	QStringList strings;
	strings.append("");		strings.append("");  strings.append(""); strings.append("");	
	//qDebug()<<"H2";
...
//        qDebug()<<"ist.count"<<list.count()<<"CurveCouunt"<<CurveCount;
	...
	{
		QString mstring;
		if (list.count()==1) mstring="this curve"; else mstring="these curves";
			...
			==QMessageBox::Ok)
		{
			...
			for (int i=0; i<list.count(); i++) {Curves.removeLast();}
...
void MainWindowImpl::ResetUndo()
{
	...
	if (UndoEvents.count()) qDeleteAll(UndoEvents.begin(), UndoEvents.end()); UndoEvents.clear();
	if (RedoEvents.count()) qDeleteAll(RedoEvents.begin(), RedoEvents.end()); RedoEvents.clear();

    //make a 'start' event
	MasksUndoDirty=true; LocksUndoDirty =true; CurvesUndoDirty=true; 
...
void MainWindowImpl::on_OOTreeWidget_itemPressed(QTreeWidgetItem* item, int column)
{
	...
	if (LastColumnClicked==column && LastItemClicked==item)
	{
		if (LastTimeClicked.msecsTo(now)<500) //double click
			{on_OOTreeWidget_itemDoubleClicked(item, column); return;}
	}
...
void MainWindowImpl::on_CurvesTreeWidget_itemPressed(QTreeWidgetItem* item, int column)
{
	...
	{
		if (LastTimeClicked.msecsTo(now)<500) //double click
			{on_CurvesTreeWidget_itemDoubleClicked(item, column); return;}
	}
...
void MainWindowImpl::on_MasksTreeWidget_itemPressed(QTreeWidgetItem* item, int column)
{
	...
	{
		if (LastTimeClicked.msecsTo(now)<500) //double click
			{on_MasksTreeWidget_itemDoubleClicked(item, column); return;}
	}
...
void MainWindowImpl::on_SegmentsTreeWidget_itemPressed(QTreeWidgetItem* item, int column)
{
	...
	{
		if (LastTimeClicked.msecsTo(now)<500) //double click
			{on_SegmentsTreeWidget_itemDoubleClicked(item, column); return;}
	}
	...
	for (int i=0; i<SegmentCount; i++)
		if (item==Segments[i]->widgetitem) //found it
		{
			if (column==4) 	Segments[i]->Locked=!(Segments[i]->Locked);
                        if (column==3) 	{Segments[i]->Activated=!(Segments[i]->Activated);        ResetFilesDirty();	rangescene->Refresh();}
			...
			if (column==2) //colour
			{
				...
				if (newcol.isValid())
				{
					Segments[i]->Colour[0]=newcol.red(); Segments[i]->Colour[1]=newcol.green(); Segments[i]->Colour[2]=newcol.blue();
				}
			}
		...
		if (item==OutputObjects[i]->widgetitem) //found it
		{
			...
			if (column==2) //Key
			{
			     ...
			     bool ok;
                             ...
                             int asc=OutputObjects[i]->Key; if (asc>=65 && asc<=90) asc-=64; else if (asc>=48 && asc<=57) asc-=21;
			   
			     QString item = QInputDialog::getItem(this, tr("Shortcut Key"),
			                                          "", items, asc, false, &ok);
			     if (ok && !item.isEmpty())
			     {
                        if (item=="[None]") OutputObjects[i]->Key=0; else OutputObjects[i]->Key=(int) (item.toLatin1()[0]);
			     }
				RefreshOneOOItem(OutputObjects[i]->widgetitem,i);
			...
			if (column==5) //merge
			{
                            if (OutputObjects[i]->IsGroup) {OutputObjects[i]->Merge = !OutputObjects[i]->Merge; ResetFilesDirty();}
				else 
				{	
					if (OutputObjects[i]->Parent!=-1)
					{
...
void MainWindowImpl::on_OONew_clicked()
{
	...
	for (int i=0; i<SegmentCount; i++)
		if (Segments[i]->widgetitem->isSelected() || (SegmentCount==1 && cflag==false))
				{o->ComponentSegments.append(i); count++; 
				if (sname=="") sname=" (" + Segments[i]->Name + ")"; else sname=" (Multiple Segs)";}

        if (count==0) {Message("Select one or more segments"); delete o; return;}
	...
	for (int j=0; j<=MaxUsedMask; j++)
	{
		if (MasksSettings[j]->widgetitem->isSelected())
		{
			...
			if (SegmentCount==1) name=MasksSettings[j]->Name; else name=MasksSettings[j]->Name + sname;
			count++;
		}
	}
	
        if (count==0 && cflag==false) {Message("Select one or more masks"); delete o; return;}
	...
	r/=countc; g/=countc; b/=countc;

	o->Colour[0]=r; o->Colour[1]=g; o->Colour[2]=b;
...
void MainWindowImpl::on_OOGroup_clicked()
{
	...
	else
	{
		...
		o->Resample=OOResample->value();
		o->IsGroup=true;			
		if (flag) o->Parent=-1; else o->Parent=oldparent;	
		int max=-1;
		for (int i=0; i<OutputObjectsCount; i++)
			if (OutputObjects[i]->ListOrder>max) max=OutputObjects[i]->ListOrder;
...
void MainWindowImpl::on_OOTreeWidget_itemSelectionChanged()
{
	...
	if (OutputRegroupMode)
	{
		...
		if (count>0) 
			{	
				OutputRegroupMode=false; OOTreeWidget->setCursor(Qt::ArrowCursor); RefreshOO(); }
		else 
		{	if (QMessageBox::question(this, "Move Object to Group", "Select a group to move selected objects to, or click Cancel to abort", QMessageBox::Ok | QMessageBox::Cancel)
			==QMessageBox::Cancel) {OutputRegroupMode=false; OOTreeWidget->setCursor(Qt::ArrowCursor);}
		}
	}
...
void MainWindowImpl::on_actionUngroup_triggered()
{
    ...
    if (flag) Message ("No objects selected that can be removed from groups!");
        else {RefreshOO(); ResetFilesDirty();}
...
void MainWindowImpl::on_actionMove_item_to_Group_triggered()
{
	...
	for (int i=0; i<OutputObjectsCount; i++)
	{
		if (OutputObjects[i]->widgetitem->isSelected()) {OutputObjects[i]->TempSelected=true; count++;} else  OutputObjects[i]->TempSelected=false;
		if (count!=0)
		{
...
{
	CopyingImpl cop;
        if (actionUse_Old_Exporting_Code->isChecked()) cop.ExportSPV(0); else cop.ExportSPV_2(0);
}
...
{
	CopyingImpl cop;
        if (actionUse_Old_Exporting_Code->isChecked()) cop.ExportSPV(1); else cop.ExportSPV_2(1);
}

void MainWindowImpl::on_actionView_in_SPIERSview_triggered()
{
        CopyingImpl cop;
        if (actionUse_Old_Exporting_Code->isChecked()) cop.ExportSPV(2); else cop.ExportSPV_2(2);
}
...
void MainWindowImpl::on_actionMaskCopy_selected_from_Previous_triggered()
{
	...
	else 
	{
		...
		//do advance/back
		if (MasksMoveForward) 
                        if (CurrentFile==FileCount-1) Message ("This is the last file - can't move forward!"); else SliderPos->setValue(CurrentFile+2);
		else if (MasksMoveBack)
                        if (CurrentFile==0) Message ("This is the first file - can't move back!"); else SliderPos->setValue(CurrentFile);
	}
}
...
{
	CopyingImpl cop;
	if (SliceSelectorList->selectedItems().count()==0 || SliceSelectorList->selectedItems().count()>1 || (SliceSelectorList->selectedItems().count()==1 && SliceSelectorList->item(CurrentFile)->isSelected()))
...
void MainWindowImpl::on_actionMask_Advance_slice_after_copy_operation_toggled(bool )
{
	if (actionMask_Advance_slice_after_copy_operation->isChecked() )
		{MasksMoveBack=false; MasksMoveForward=true; actionMaskGo_back_one_slice_after_copy->setChecked(false);}
...
void MainWindowImpl::on_actionMaskGo_back_one_slice_after_copy_toggled(bool )
{
	if (actionMaskGo_back_one_slice_after_copy->isChecked() )
		{MasksMoveForward=false; MasksMoveBack=true; actionMask_Advance_slice_after_copy_operation->setChecked(false);}
...
//scroll bar moved - change the current image
{
	...
	if (HorribleBodgeFlagDontStoreUndo==false)
	{
		...
		UndoEvents.prepend(new UndoEvent(-1,-1));
		qDeleteAll(RedoEvents.begin(), RedoEvents.end()); RedoEvents.clear(); //wipe the redo
		if (RedoEvents.count()>0) actionRedo->setEnabled(true); else actionRedo->setEnabled(false);
	}
...
void MainWindowImpl::FixUpStretches()
{
	...
	for (int i=1; i<FullFiles.count(); i++)
	{
		if (FullStretches[i]<=FullStretches[i-1])
			{ if (affectedslice==-1) affectedslice=i; do FullStretches[i]++; while (FullStretches[i]<=FullStretches[i-1]);}
	}
	...
	if (affectedslice!=-1) 
	{
		...
		if (QMessageBox::warning(this, "Change Slice Spacing", mess, QMessageBox::Ok | QMessageBox::Cancel)
			!=QMessageBox::Ok)
		{FullStretches=OldStretches; return;}
	}
...
void MainWindowImpl::on_actionChange_slice_spacing_triggered()
{
	...
	if (SliceSelectorList->selectedItems().count()==0) {Message("No slices selected!");return;}
...
void MainWindowImpl::on_actionSet_slice_position_triggered()
{
	...
	int firstsel=-1;
	for (int i=0; i<FileCount; i++)
		if (SliceSelectorList->item(i)->isSelected()) {firstsel=i; break;}	
	
	if (firstsel==-1) Error ("Internal error - failed to find selected slice");
	if (firstsel==0) lowvalue=0; else lowvalue=FullStretches[(firstsel-1)*zsparsity]*(1.0/SlicePerMM);
...
void MainWindowImpl::on_actionInterpolate_over_selected_slices_triggered()
{
	...
	else
	{
		...
		else
		{
			...
			FirstFile*=zsparsity;
			LastFile*=zsparsity;
			if (FirstFile+1>=LastFile) {Message("Nothing to interpolate!"); return;}
			...
			{
				if (QMessageBox::question(this, "Interpolate Curves", "This will interpolate selected curves between first and last selected slices - any data in intervening slices will be overwritten. Are you sure?", QMessageBox::Yes | QMessageBox::Cancel)
				==QMessageBox::Yes)
				{
