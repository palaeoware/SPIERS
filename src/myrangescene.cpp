...
SPIERSedit 2: myrangescene.cpp

Subclassed QGraphicsScene object for scene
handling range generation
...
int myrangescene::closesttop(int x, int *dist)
{
	...
	{
		if (qAbs(Segments[i]->RangeTop - x) <d) {c=i; d=	qAbs(Segments[i]->RangeTop - x);}
	}
	*dist=d;
	return c;
}
...
int myrangescene::closestbase(int x, int *dist)
{
	...
	if (Segments[i]->Activated)
	{
		if (qAbs(Segments[i]->RangeBase - x) <d) {c=i; d=	qAbs(Segments[i]->RangeBase - x);}
	}
	*dist=d;
	return c;
}
	...
	if (PressedButton>=1) 
	{
		...
		if (button==1) //do snapping
		{
			if (selectedtop>=0 && selectedbottom==-1) //can snap to a baseline
			{
				...
				if (Segments[i]->Activated)
				{
					if ((Segments[i]->RangeBase-x)>0 && ((Segments[i]->RangeBase-x)<min)) {t=i;min=	Segments[i]->RangeBase-x;}
				}
			...
			if (selectedbottom>=0 && selectedtop==-1) //can snap to a topline
			{
				...
				if (Segments[i]->Activated)
				{
					if ((x-Segments[i]->RangeTop)>0 && ((x-Segments[i]->RangeTop)<min)) {min=x-Segments[i]->RangeTop; t=i;}
				}
				//min is now distance up to closest 
		...
		if (selectedtop>=0) {x=LimitTop(x); if (selectedbottom>=0) x=LimitBase(x); Segments[selectedtop]->RangeTop=x; flag=true;}
		if (selectedbottom>=0) {x=LimitBase(x); Segments[selectedbottom]->RangeBase=x; flag=true;}
