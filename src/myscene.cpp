...
void myscene::DoMouse(int x,int y,int PressedButton)
{
	if (x!=LastMouseX || y!=LastMouseY) // an actual move
	{
		...
		MoveFlag=true;
		if (button>0) {Counter2++; DoAction(LastMouseX,LastMouseY);} //else ChangeFlag=true;
	}
	else
	{
		...
		if (PressedButton==0 && button!=0) {MouseUp(); return;} //no button
		//catch new button presses
		if (PressedButton == 1 && button==0) 
			{ Counter2=0; button=1;  DoAction(LastMouseX, LastMouseY); return;}	
 
	    if (PressedButton == 2 && button==0)
				{ Counter2=0; button=2;  DoAction(LastMouseX, LastMouseY); return;}	

		//catch button swaps
		if (PressedButton == 1  && button==2) 
			{MouseUp(); button=1;  DoAction(LastMouseX, LastMouseY); return;}
		if (PressedButton == 2  && button==1) 
			{MouseUp(); button=2;  DoAction(LastMouseX, LastMouseY); return;}
	}
}
