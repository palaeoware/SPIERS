...
{
	if (to==from) if (f==to) return true; else return false;
	if (to>=from)
	{
		if (f<=to) return true; else return false;	
	}
	else
	{	
		if (f>=to) return true; else return false;	
   ...
   //do mirroring by setting loop parameters
    ...
    for (f = loopfrom; freached(f,loopto,loopfrom); f+=loopstep) //loop through all files
    {
        ...
        if (*Count==-1)
        {
              ...
              {
                   int ymax=gysize+GRID_SCALE; if (ymax>aheight) ymax=aheight; //height of cell (might be less than grid size at end)
                   for (n=0; n<gxsize; n++)  //for each column
                   {
                       int xmax=gxsize+GRID_SCALE; if (xmax>awidth) xmax=awidth; //width of cell
                       ...
                       for (int mm=gysize*GRID_SCALE; mm<ymax; mm++) //for each row
                       {
                           ...
                           for (int nn=gxsize*GRID_SCALE; nn<xmax; nn++) //row each column
                               if (OutputArray[bigpos++]){ GridArray[Gpos]=255; goto break2;  }  //if I hit an on pixel turn grid cell on and stop
                       }
                break2:
                       Gpos++;
                   }
